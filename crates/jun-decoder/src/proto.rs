...
/// Signatures intentionally not captured — they're client-verifiable against the
/// tx digest + committee pubkeys, and would otherwise require path-specific
/// handling (archive format stores them under CheckpointContents, not per-tx).
...
/// Aggregated BLS signature from the validator committee certifying the checkpoint.
pub struct ProtoValidatorAggSig<'a> {
    pub epoch: u64,
    pub signature: &'a [u8],
    pub bitmap: &'a [u8],
}

/// Parsed checkpoint proto — BCS byte slices + validator signature.
pub struct ProtoCheckpoint<'a> {
    ...
    pub contents_bcs: Option<&'a [u8]>,
    pub validator_signature: Option<ProtoValidatorAggSig<'a>>,
}
...
pub fn parse_checkpoint(buf: &[u8]) -> Result<ProtoCheckpoint<'_>, &'static str> {
    ...
    while pos < buf.len() {
        ...
        match (field_number, wire_type) {
            ...
            // field 4: signature (ValidatorAggregatedSignature)
            (4, 2) => {
                let (data, new_pos) = read_length_delimited(buf, pos)?;
                pos = new_pos;
                result.validator_signature = parse_validator_agg_sig(data);
            }
            // field 5: contents (CheckpointContents with nested bcs field)
            (5, 2) => {
                let (data, new_pos) = read_length_delimited(buf, pos)?;
                pos = new_pos;
                result.contents_bcs = extract_bcs_value(data);
            }
...
/// Parse a ValidatorAggregatedSignature message: epoch / signature / bitmap.
fn parse_validator_agg_sig(buf: &[u8]) -> Option<ProtoValidatorAggSig<'_>> {
    let mut epoch = 0u64;
